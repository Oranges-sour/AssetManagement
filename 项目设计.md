...
在该部门下新增位置空间（A-301，60.5㎡）

新增领用人（E1001 张三）

新增资产（AS0001 笔记本电脑，位置 A-301，初始闲置）

资产领用：对 AS0001 执行 assign → 绑定张三，状态变为领用

资产归还：对 AS0001 执行 return → 领用人置空，状态变为闲置
...
8.2 部署步骤（概述）
创建数据库 asset_db，执行建表 SQL

配置数据库连接信息（如 db.properties）

Maven 打包为 war

将 war 放入 Tomcat 的 webapps 并启动
...
status：0闲置/1领用（与 assignee_id 保持一致）
